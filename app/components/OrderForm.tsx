...
const empty:Draft={eventType:"",name:"",contact:"",eventDate:"",city:"",guestCount:"",budget:"PREMIUM — 4 990 ₽",selectedStyle:"Кинематографичный",modules:["Ответы гостей","Карта и программа"],musicMood:"Деликатный фон",message:"",consent:false,website:""};
...
const draftStorageKey="pre-order-draft-v1";
...
export function OrderForm(){
  ...
  const submit=async(e:FormEvent)=>{e.preventDefault(); if(!draft.consent||!draft.eventType)return; setStatus("sending"); try{
    const response=await fetch("/api/leads",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...draft,guestCount:Number(draft.guestCount||0),source:"website",idempotencyKey:crypto.randomUUID()})});
    if(!response.ok)throw new Error();
    setStatus("success"); localStorage.removeItem(draftStorageKey);
  }catch{setStatus("error")}};
  if(status==="success")return <div className="order-success" role="status" aria-live="polite"><span>✓</span><p>БРИФ ПОЛУЧЕН</p><h1>Первый кадр<br/>уже случился</h1><p>Мы изучим детали и свяжемся с вами в течение рабочего дня</p><Link href="/">Вернуться на главную ↗</Link></div>;
  return <form className="order-form" onSubmit={submit}>
    ...
    {step===1&&<section><p>ПОВОД · ВЫБЕРИТЕ ОДИН</p><h1>Что будем<br/>предвкушать?</h1><div className="occasion-picks">{occasionList.map(item=><button type="button" aria-pressed={draft.eventType===item.slug} className={draft.eventType===item.slug?"active":""} onClick={()=>patch("eventType",item.slug)} key={item.slug}>{item.short}<b>↗</b></button>)}</div>{!draft.eventType&&<p className="form-hint">Выберите повод, чтобы продолжить</p>}</section>}
